...
def main(model_name,num_mol_filter,mol_display):
    latent_reps =np.load('latent_reps292_2113.npy')
    print type(latent_reps),latent_reps.shape
    matrix =sp.load_npz('136rescale.npz')
...
#    binary =binarymtx(matrix,bin_list)
    binary = sp.load_npz('136binarymtx.npz')
...
def everything(model_name,num_mol_filter,mol_display): 
    ...
    filename = model_name +','+str(num_mol_filter)+','+str(mol_display)+',results-136.csv'

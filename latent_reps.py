...
def get_latent_rep(smiles_list,data_file,model_file,model):
  charset = load_charset(data_file)
  model.load(charset, model_file)
  canonicalised_smiles = [Chem.MolToSmiles(Chem.MolFromSmiles(str(x))) for x in smiles_list]
  input_array = get_input_arr(canonicalised_smiles, charset)
  latent_reps = encode(model, input_array)
  return latent_reps
...
# np.save('latent_reps292.npy',latent_reps)
